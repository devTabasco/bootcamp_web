...
--R: 정상(사용자요청) I: 비정상
ALTER TABLE ACCESSLOG
ADD AC_BROWSER NVARCHAR2(50) NOT NULL
ADD AC_PUBLICIP NVARCHAR2(15) NOT NULL
ADD AC_STATE NCHAR(1) NOT NULL;

select a.sid, a.serial# 
from v$session a, v$lock b, dba_objects c 
where a.sid = b.sid and b.id1=c.object_id and b.type='TM' and c.object_name='ACCESSLOG';

alter system kill session '91, 1343';

alter session set ddl_lock_timeout = 600;

CREATE OR REPLACE VIEW ISACCESS
		AS
		SELECT AC_SESTCODE AS STORECODE, AC_SECODE AS EMPCODE, AC_TYPE AS ISACCESS
		FROM AC
		WHERE (SYSDATE - AC_DATE) * 24 < 24 
  		  AND (AC_SESTCODE, AC_SECODE, AC_DATE) IN (SELECT AC_SESTCODE, AC_SECODE, MAX(AC_DATE) AS ACCESSDATE
        				  FROM AC 
          				  WHERE (SYSDATE - AC_DATE) * 24 < 24
                  		  GROUP BY AC_SESTCODE, AC_SECODE);
                          
SELECT ISACCESS  FROM ISACCESS WHERE STORECODE = '1234567890' AND EMPCODE = '000';

delete
from AC;

commit;
ALTER TABLE ACCESSLOG MODIFY AC_TYPE NUMBER(1);

AC_SESTCODE_SECODE_DATE_PK

ALTER TABLE ACCESSLOG
DROP CONSTRAINT AC_SESTCODE_SECODE_DATE_PK;

ALTER TABLE ACCESSLOG
ADD CONSTRAINT SESTCODE_SECODE_DATE_TYPE_PK
PRIMARY KEY (AC_SESTCODE, AC_SECODE, AC_DATE, AC_TYPE);