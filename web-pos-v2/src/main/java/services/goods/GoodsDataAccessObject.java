...
public class GoodsDataAccessObject extends DataAccessObject{
	...
	int insGoods(Connection connect, StoreBean clientData) {
		int result = 0;
		String dml = "INSERT INTO SM(SM_STCODE, SM_CODE, SM_SCCAT, SM_NAME, SM_SCSTATE, SM_COLOR, SM_SICODE) "
				+ "VALUES ( ?, ?, ?, ?, ?, ?, ?)";
		try {
			this.ps = connect.prepareStatement(dml);
			this.ps.setNString(1, clientData.getStoreCode());
			this.ps.setNString(2, clientData.getGoodsList().get(0).getGoodsCode());
			this.ps.setNString(3, clientData.getGoodsList().get(0).getGoodsCategoryCode());
			this.ps.setNString(4, clientData.getGoodsList().get(0).getGoodsName());
			this.ps.setNString(5, clientData.getGoodsList().get(0).getGoodsStateCode());
			this.ps.setNString(6, clientData.getGoodsList().get(0).getGoodsColorCode());
			this.ps.setNString(7, clientData.getGoodsList().get(0).getGoodsImageCode());
	...
	int insImages(Connection connect, ArrayList<StoreImages> clientData) {
		int result = 0;
		String dml = "INSERT INTO SI(SI_CODE, SI_NAME, SI_LOC) "
				+ "VALUES ( ?, ?, ?)";
		try {
			this.ps = connect.prepareStatement(dml);
			for(StoreImages image : clientData) {
				this.ps.setNString(1, image.getImageCode());
				this.ps.setNString(2, image.getImageDisplayName());
				this.ps.setNString(3, image.getImageLocation());
