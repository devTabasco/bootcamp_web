...
public class GoodsManagement {
	...
	public ActionBean backController(int serviceCode) {
		...
		if(this.session.getAttribute("AccessInfo") != null) {
			...
			try {
				action.setPage("index.jsp?message="+ URLEncoder.encode("에러메세지:로그인 후 사용하실 수 있습니다.", "UTF-8"));
	...
	private ActionBean insGoodsCtl() {
		...
		boolean isForward = true; boolean tran = false;
		...
		System.out.println((store.getGoodsList() == null)+"확인");
		...
		if(ServletFileUpload.isMultipartContent(this.req)) {		
			...
			store.getGoodsList().get(store.getGoodsList().size()-1).setGoodsColorCode(this.req.getParameter("goodsColorCode").substring(1));
		}
		...
		if(this.convertToBoolean(this.dao.insGoods(connect, store))) {
			tran = this.convertToBoolean(this.dao.insImages(connect, storeImageList));
		}
