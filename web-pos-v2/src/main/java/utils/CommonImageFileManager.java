...
public class CommonImageFileManager {
	...
	public ArrayList<StoreImages> saveGoodsImageFile(StoreBean store, String groupCode, boolean isThumbnail) {
		...
		String savePath = "D:\\Dynamic_JS\\CodeSpace\\web-pos-v2\\src\\main\\webapp\\resources\\images\\"+ groupCode;
		/* StoreImages 생성 */
		...
		try {
			for(FileItem file : files) {
				if(file.isFormField()) {
					...
					case "goodsColorCode": store.getGoodsList().get(store.getGoodsList().size()-1).setGoodsColorCode(file.getString("UTF-8").substring(1));break;
