...
<body onLoad="pageInit('${param.message}', '${AccessInfo.storeInfoList[0].empList[0].accessList[0].accessDate }')">
	...
	<script>
		...
		function mgrInit() {
			...
			//alert(jsonString);
			/* S : 이전 선택 환경 복구 */
			selectedIdx = ('${param.currentIdx}' == '')? '${currentIdx}'.split(":") : '${param.currentIdx}'.split(":");
		...
		function subMenuClick() {
			...
			let itemCodeIdx = null;
			
			if(selectedIdx.length > 3 && subMenuIdx == 5){
				itemCodeIdx = subMenuIdx + parseInt(selectedIdx[3]);
			}else{
				itemCodeIdx = subMenuIdx + ((subMenuIdx > 5)? 6 : 0);
			}
		...
		function goodsRegistrationCtl(){
			...
			if(jsonData == null){
				...
				if(jsonData.goodsList != null){
					goodsSeq = parseInt(jsonData.goodsList[0].goodsCode.substr(1,2));
					for(idx=1; idx<jsonData.goodsList.length; idx++){
						if(goodsSeq < parseInt(jsonData.goodsList[idx].goodsCode.substr(1,2))){
							goodsSeq = parseInt(jsonData.goodsList[idx].goodsCode.substr(1,2));
						}
					}
					goodsCode = "G" + ((goodsSeq<9)? "0" + (goodsSeq+1) : goodsSeq+1); 
				}
				/* 상품코드 표시 Object */
				...
				/* 상품이름 입력 Object */
				const gnBox = createInputBox("text", "goodsInfo", "", "상품이름");
				gnBox.setAttribute("class", "item");
				itemList.appendChild(gnBox);
				...
				/* 상품이미지 선택 */
				const gnImage = createFileBox("goodsImageCode");
		...
		function goodsRegistration(){
			...
			for(idx=0; idx<goodsInfo.length; idx++){
				formChild.push(createInputBox("hidden", 
						itemCode[recordIdx][5][1][idx],	goodsInfo[idx].value, ""));
			}
			formChild.push(document.getElementsByName("goodsImageCode")[0]);
