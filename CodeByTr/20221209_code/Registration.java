...
public class Registration {
	...
	private ActionBean regStoreCtl() {
		...
		/* StoreInfo*/
		group.getStoreInfoList().get(0).setStoreCode(this.req.getParameter("storeCode"));
		group.getStoreInfoList().get(0).setStoreName(this.req.getParameter("storeName"));
		group.getStoreInfoList().get(0).setStorePhone(this.req.getParameter("storePhone"));
		group.getStoreInfoList().get(0).setStoreZip(this.req.getParameter("storeZip"));
		group.getStoreInfoList().get(0).setStoreAddr(this.req.getParameter("storeAddr"));
		group.getStoreInfoList().get(0).setStoreAddrDetail(this.req.getParameter("storeAddrDetail"));
		/* Default CategoriesInfo*/
		for(String[] lev : levInfo) {
			group.getStoreInfoList().get(0).getLevInfo().get(0).setLevCode(lev[0]);
			group.getStoreInfoList().get(0).getLevInfo().get(0).setLevName(lev[1]);
		}
		...
		if(this.convertToBoolean(this.dao.insStore(connection, group))) {
			if(this.convertToBoolean(this.dao.insStoreLevel(connection, group))) {
				...
				this.req.setAttribute("levInfo", this.mekeHTMLSelectBox(group));
	...
	private String mekeHTMLSelectBox(Object obj) {
		StringBuffer select = new StringBuffer();
		select.append("<select name=\"levelInfo\" class=\"communicationBox tripple\">");
