...
public class Registration {
	...
	private ActionBean groupNameDuplicateCheckCtl() {
		...
		if(groupList == null) {
			if(this.convertToBoolean(dao.insTemp(this.connect, group))) {
				...
				page = "grStep2.jsp";
				forward = true;
			}else {message = "네트워크 에러:네트워크가 불안정합니다. 잠시 후 다시 시도해주세요:1";}
		}else {message = "그룹명 중복:이미 사용중인 그룹명입니다.:1";}		
	...
	private ActionBean delGroupNameCtl() {
		...
		if(!page.equals("index.jsp")){
			...
			/* 2. DAO :: DML */
			this.dao = new RegDataAccessObject();
			if(connectUser)  this.connect = this.dao.openConnection("WEBDBA", "8520");
			else  this.connect = this.dao.openConnection();
