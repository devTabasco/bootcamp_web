...
<!--  -->
<body
	onLoad="pageInit('${param.message}', '${AccessInfo.storeInfoList[0].empList[0].accessList[0].accessDateTime }','${param.current }')">
	<div id="wrap">
		<div id="side">
			<div id="store">${options}</div>
			<div id="emp">
				<span>${AccessInfo.storeInfoList[0].empList[0].empName }</span><span>
					${AccessInfo.storeInfoList[0].levInfo[0].levName }</span>
			</div>
			<div id="menu">
				...
				<ul>
					<li><span
						onClick="subMenuClick(5);selelctCategoryCtl('${AccessInfo.storeInfoList[0].storeCode}');">분류관리</span></li>
					<li><span
						onClick="subMenuClick(6);selectGoodsManagement('${AccessInfo.storeInfoList[0].storeCode}');">상품관리</span></li>
					<li><span
						onClick="subMenuClick(7);selectCostPriceManagement('${AccessInfo.storeInfoList[0].storeCode}');">가격관리</span></li>
				</ul>
	...
	<div class="communicationBox single" style="display: none">
		<div id="inputZone">
			<input type="text" name="storeCode" placeholder="매장코드"
				class="box big" />
		</div>
	</div>
	...
	<script>
		const itemCode = [ [ "C", "상품분류" ], [ "S", "상품상태" ], [ "L", "직원등급" ],
				[ "D", "가격분류" ], [ "Z", "구역분류" ], [ "O", "주문상태" ],
				[ "P", "결제상태" ] ];
		const menuList = [ "종합 분석", "일자별 분석", "상품별 분석", "결재 수단별 분석", "고객별 분석",
				"상품 분류 관리", "상품 정보 관리", "가격 정보 관리", "판매 구역 관리", "테이블 등록",
				"테이블배치", "주문 상태 관리", "결제 상태 관리", "직원 정보 관리", "고객 정보 관리" ];
		let currentIdx = null;
		...
		function menuClick(idx) {
			...
			if (subMenu[idx].style.display === "block"){
				//열려있는걸 닫는 경우
				menu[idx].style.color = "rgba(255, 255, 255, 1)";
				subMenu[idx].style.display = "none";
				
				const mainTitle = document.getElementById("selectedMenu"); //<div id="selectedMenu">서브 메뉴를 선택해 주세요</div>
				mainTitle.innerText = "서브 메뉴를 선택해 주세요";
			}else{
		...
		function selectCategoryStep1(storeCode) {
			...
			for (let idx = 0; idx < 7; idx++) {
				const button = createDIV("", "item", "serverCall('" + storeCode
						+ "', '" + itemCode[idx][0] + "', " + idx + ", '"
						+ (currentIdx + ":" + storeCode + ":" + idx) + "')");
		...
		function serverCall(storeCode, itemCode, objIdx, cIdx) {
			...
			if (storeCode != '') {
				...
				form.appendChild(createInputBox("hidden", "storeCode",
						storeCode, ""));
